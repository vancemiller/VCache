...
void Read(ADDRESS addr, UINT32 size) {
  ...
  for (std::vector<CacheLine*>::iterator it = lines.begin(); it != lines.end();
      it++) {
    if ((*it) == NULL) {
      std::cout << "Dereferencing NULL!" << std::endl;
    }
    (*it)->Read(addr, size);
  }
}
...
void Write(ADDRESS addr, UINT32 size) {
  std::cout << "Write " << addr << " " << size << std::endl;
  std::vector<CacheLine*> lines = mlc->Access(addr, size);
  std::cout << "Access completed" << std::endl;
  for (std::vector<CacheLine*>::iterator it = lines.begin(); it != lines.end();
      it++) {
    if ((*it) == NULL) {
      std::cout << "Dereferencing NULL!" << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  uint32_t line_size = KnobLineSize.Value();
  ...
  mlc = new MultilevelCache(capacities_B, associativities, line_size);
